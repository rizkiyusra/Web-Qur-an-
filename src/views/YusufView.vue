...
export default {
  ...
  methods: {
    async getChapter() {
      fetch("https://api.quran.com/api/v4/chapters/12?language=id", {
        method: "GET",
      })
        .then((response) => {
          if (response.ok) {
    ...
    async getChapterInfo() {
      fetch("https://api.quran.com/api/v4/chapters/12/info?language=id", {
        method: "GET",
      })
        .then((response) => {
          if (response.ok) {
    ...
    async getAudio_File() {
      fetch("https://api.quran.com/api/v4/chapter_recitations/7/12", {
        method: "GET",
      })
        .then((response) => {
          if (response.ok) {
    ...
    async getTranslation() {
      fetch(
        "https://api.quran.com/api/v4/quran/translations/33?fields=id&chapter_number=12",
        {
          method: "GET",
        }
      )
